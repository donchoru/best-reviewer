...
God Class를 분해한 각 클래스(Config, DocumentLoader, TextChunker,
VectorStore, RAGPipeline)를 독립적으로 테스트한다.
...
class TestDocumentLoader(unittest.TestCase):
    ...
    def test_load_csv_parses_rows_into_text(self):
        """CSV 파일의 각 행이 'key: value' 파이프 구분 형식으로 변환되는지 확인."""
        with tempfile.NamedTemporaryFile(
    ...
    def test_unsupported_type_raises_value_error(self):
        """지원하지 않는 소스 타입('xml') 요청 시 ValueError가 발생하는지 확인."""
...
class TestPipeline(unittest.TestCase):
    ...
    Embedder의 외부 API 호출을 Mock으로 대체하여 네트워크 없이 테스트한다.
    """
    ...
    def test_batch_counts_success_and_fail(self, mock_post):
        """배치 처리에서 성공 1건 + 미지원 타입 실패 1건이 정확한지 확인."""
        mock_post.return_value = MagicMock(
            status_code=200,
            json=lambda: {"embedding": {"values": [0.1] * 768}})
