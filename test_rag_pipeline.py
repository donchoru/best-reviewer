...
from stores.similarity import (
    BaseSimilarity, CosineSimilarity, EuclideanSimilarity, DotSimilarity, get_similarity,
)
...
class TestSimilarity(unittest.TestCase):
    ...
    def test_dot_identical_unit_vectors(self):
        """내적: 동일 단위 벡터 → 1.0 확인."""
        self.assertAlmostEqual(DotSimilarity().calculate([1, 0, 0], [1, 0, 0]), 1.0, places=3)
    def test_dot_orthogonal_vectors(self):
        """내적: 직교 벡터 → 0.0 확인."""
        self.assertAlmostEqual(DotSimilarity().calculate([1, 0], [0, 1]), 0.0, places=3)
    def test_get_similarity_returns_correct_instance(self):
        ...
        self.assertIsInstance(get_similarity("dot"), DotSimilarity)
...
class TestVectorStore(unittest.TestCase):
    ...
    def test_save_chunks_and_search_returns_match(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            store.close()
    def test_store_with_euclidean_similarity_di(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            store.close()
    def test_store_with_dot_similarity_di(self):
        """DI로 DotSimilarity 주입 후 검색 동작 확인."""
        with tempfile.TemporaryDirectory() as d:
            store = SqliteVectorStore(
                StoreConfig(db_path=os.path.join(d, "t.db")),
